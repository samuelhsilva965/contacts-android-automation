...
class ValidationNewCrateContact:
    ...
    def validate_create_new_contact(self, first_name: str = "", last_name: str = "", phone: str = "", email: str = ""):
        ...
        assert self.contact_detail_page.get_btn_add_to_favorites().is_enabled(), "O botão de Favoritos não está habilitado"
        assert self.contact_detail_page.get_btn_edit().is_enabled(), "O botão de Editar não está habilitado"
        assert self.contact_detail_page.get_btn_more_options().is_enabled(), "O botão de Mais Opções não está habilitado"

        # --- 3. VALIDAÇÃO DO CAMPO DE TELEFONE ---
        if phone != "":
            phone_from_screen = self.contact_detail_page.get_text_phone_number().text
            phone_clean = re.sub(r'\D', '', phone_from_screen) # Remove parênteses e traços
            
            assert phone_clean == phone, f"Telefone incorreto: Esperava '{phone}', mas leu '{phone_clean}'"
            assert self.contact_detail_page.get_btn_send_msg().is_enabled(), "Botão de enviar SMS não está habilitado"
